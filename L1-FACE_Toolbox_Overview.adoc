...
=== Standard Artefacts
One of the inventory tools is a standard artefact mask. The lab may not have a wide range of faces to use for testing. Standard artefacts are a way to expand the number of possible test users without needing to bring in additional people. 

Standard artefacts can be used at the discretion of the lab. Testing cannot be completed solely with the use of standard artefacts though, live users must be included in the data set. The use of standard artefacts shall be documented in the test report.
...
=== Pass/Fail Criteria with Standard Artefacts
Standard artefacts used in calculating the IAPAR cannot be the only artefact to fail the test causing the failure. If the only failures are from standard artefacts and the results cannot be replicated in live users, then the test is considered a pass.
